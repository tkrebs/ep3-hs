            ...
            if ($loginForm->isValid()) {
                ...
                switch ($loginResult->getCode()) {
                    case Result::SUCCESS:
                        ...
                        $origin = $this->redirectBack()->getOrigin();

                        if ($origin && $origin['route'] != 'frontend') {
                            return $this->redirectBack()->toOrigin();
                        } else {
                            if ($user->can('admin')) {
                                return $this->redirect()->toRoute('backend/dashboard');
                            } else {
                    ...
                    case Result::FAILURE_USER_STATUS:
                        ...
                        switch ($user->need('status')) {
                            case 'placeholder':
                                ...
                                break;
                            case 'guest':
                                $loginMessage = 'This account is considered a guest account and thus cannot login';
                                break;
                            case 'deleted':
                            ...
                            case 'blocked':
                                ...
                                break;
                            case 'disabled':
                                $loginMessage = 'This account is currently disabled';
                                break;
                        }
